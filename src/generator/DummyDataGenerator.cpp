...
int DummyDataGenerator::nextIdSuffix() const {
    int maxSuffix = 0;
    for (const auto& sample : repository_.readAll()) {
        if (sample.id.rfind("S-", 0) == 0) {
            try {
                int suffix = std::stoi(sample.id.substr(2));
                maxSuffix = std::max(maxSuffix, suffix);
            } catch (const std::exception&) {
                // Ignore ids that don't follow the "S-###" convention.
            }
        }
    }
    return maxSuffix + 1;
...
Sample DummyDataGenerator::generateOne(int idSuffix) {
    ...
    std::uniform_real_distribution<double> processTimeDist(0.1, 1.0);
    std::uniform_real_distribution<double> yieldDist(0.70, 0.99);
    std::uniform_int_distribution<int> stockDist(0, 1000);

    Sample sample;
    char idBuffer[16];
    std::snprintf(idBuffer, sizeof(idBuffer), "S-%03d", idSuffix);
    sample.id = idBuffer;
